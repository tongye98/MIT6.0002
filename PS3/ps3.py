...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        # raise NotImplementedError
        m = random.random() + random.randint(0,self.width-1)
        n = random.random() + random.randint(0,self.height-1)
        return Position(m,n)
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        m = math.floor(pos.get_x())
        n = math.floor(pos.get_y())
        if (m,n) in self.tiles.keys() and (m,n) not in self.furniture_tiles:
            return True
        else: return False
    ...
    def get_random_position(self):
        ...
        while 1:
            m = random.random() + random.randint(0,self.width-1)
            n = random.random() + random.randint(0,self.height-1)
            if self.is_position_valid(Position(m,n)):
                return Position(m,n)
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        new_position = position.get_new_position(self.direction,self.speed)
        if self.room.is_position_valid(new_position):
            ...
            self.set_robot_direction(direction=random.randint(0,360))
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        while room.get_num_cleaned_tiles() / room.get_num_tiles() < min_coverage:
            #print(room.tiles)
            #print('cleaned tiles = {}'.format(room.get_num_cleaned_tiles()))
            time_step += 1
            for robot in robots:
                #print('position:{}'.format(robot.get_robot_position()))
